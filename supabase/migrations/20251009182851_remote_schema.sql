...
alter table "public"."applications" add column "resume_path" text not null;
...
create or replace view "public"."company_applications" as  SELECT jobs.company_id,
    count(jobs.company_id) AS num_applications
   FROM (applications
...
with check ((bucket_id = 'applications'::text));
  ...
  for delete
  to authenticated
using (((bucket_id = 'resumes'::text) AND ((storage.foldername(name))[1] = (auth.uid())::text) AND (auth.role() = 'authenticated'::text)));
...
using (((bucket_id = 'resumes'::text) AND ((storage.foldername(name))[1] = (auth.uid())::text) AND (auth.role() = 'authenticated'::text)));
  ...
  for insert
  to authenticated
with check (((bucket_id = 'resumes'::text) AND ((storage.foldername(name))[1] = (auth.uid())::text) AND (auth.role() = 'authenticated'::text)));
